...
class DefaultLoginRepository(
    ...
    override suspend fun login(loginType: LoginType): BaseResult<Unit> = when (loginType) {
        is LoginType.LoginWithPassword -> loginWithPassword(loginType)
    }

    private suspend fun loginWithPassword(loginType: LoginType.LoginWithPassword): BaseResult<Unit> =
        loginNetwork.loginUser(username = loginType.username, password = loginType.password).toBaseResult()
}